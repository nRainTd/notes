...
从上面分析出的 `pkcs#7` 填充逻辑，以及下面这段报错信息的提示，我们很容易知道分组加密模式为 `ecb`，及将数据按每块 `16` 字节等分，最后一块不足的位置填充上不足的长度 (例如，如果缺了 9 个字节，这 9 个字节都填充上 `0x09`)；如果不缺，则单独增加一块，全部填充上 `0x10`(十进制 `16`) #知识/ecb
